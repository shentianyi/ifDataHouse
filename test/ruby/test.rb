...
class Test
  ...
  def test_add_product_inspect_pack
    ...
    while i<1
      i+=1
      @productNrs=[]
      300.times { @productNrs<<UUID.generate }
      r=Random.new
      @productNrs.each_with_index do |value, index|
        entity=@entities[rand(@entities.count)]
        type=rand(index+1)%2
        @client.addProductInspect($access_key, {"entityId" => entity.entityId, "inspectTime" => ((Time.now+r.rand(-10..10)*24*3600).to_ms).to_s, "productNr" => value, "partNr" => '91G942101', "type" => type.to_s})
